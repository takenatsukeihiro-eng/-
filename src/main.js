...
function preparePrint(holes) {
  let tableRows = holes.map(hole => {
    const pinData = getPinPosition(hole.number, selectedDate);
    if (!pinData) {
      return `<tr><td>${hole.number}</td><td>${hole.par}</td><td colspan="3">未設定</td></tr>`;
    }
    
    const { greenType, position: pin } = pinData;
    const yardage = greenType === 'A' ? hole.yardageA : hole.yardageB;
    const pos = `手前/奥: ${Math.round(250 - pin.y)}, 左/右: ${Math.round(pin.x - 250)}`;
  ...
  const typeName = holes[0].number === 1 ? 'OUT' : 'IN';

  printAreaEl.innerHTML = `
    <div class="print-page">
      <div class="print-header">
        <h1>ピンポジション表 (${typeName})</h1>
        <p>勝浦東急ゴルフコース - ${selectedDate.replace(/-/g, '/')}</p>
      </div>
